...
import { ProductGridProvider } from "@/lib/product-grid-context";
...
export default function ProductsPage() {
      ...
      .then(({ data: settings }) => {
        if (settings) {
          ...
          if (settings.theme_config && typeof settings.theme_config === "object" && (settings.theme_config as any).global?.colors) {
            ...
            c = ensureSystemPageInConfig(c, "/products");
            setConfig(c);
          } else {
  ...
  const activeSections = enforceFooterAtEnd(getPageSections(c, "/products").filter((s) => !s.disabled));
