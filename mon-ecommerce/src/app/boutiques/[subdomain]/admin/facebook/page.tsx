...
export default function FacebookChannelPage() {
  ...
  useEffect(() => {
    if (activeTab !== "performance" || !connected) return;
    const fetchInsights = async () => {
      setInsightsLoading(true);
      try {
        const res = await fetch(`/api/meta/insights?days=7&shop_slug=${subdomain}`);
        const data = await res.json();
        if (data.success && data.insights?.length > 0) setInsights(data.insights);
  ...
  const handleSave = async () => {
    ...
    if (existing?.id) {
      ...
      await supabase.from("settings").insert([{ ...metaSettings, shop_slug: subdomain, user_id: session?.user?.id }]);
    }
  ...
  return (
    <div className="space-y-4 sm:space-y-6 max-w-5xl">
      ...
      <div className="flex items-center gap-3 sm:gap-4">
        <div className="w-10 h-10 sm:w-12 sm:h-12 bg-blue-50 rounded-2xl flex items-center justify-center flex-shrink-0">
          <Share2 className="w-5 h-5 sm:w-7 sm:h-7 text-blue-600" />
        </div>
        <div>
          <h1 className="text-lg sm:text-xl font-bold text-gray-900">Facebook & Instagram</h1>
          <p className="text-xs sm:text-sm text-gray-500">Vendez sur Facebook et Instagram</p>
        </div>
      </div>
      ...
      {activeTab === "performance" && (
        <div className="space-y-5">
          {(() => {
            const data = insights.length > 0 ? insights : MOCK_INSIGHTS;
            ...
            }), { impressions: 0, reach: 0, clicks: 0, spend: 0, sales: 0 });
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            {[
              { label: "Impressions", value: format(totals.impressions), icon: Eye },
              { label: "Reach", value: format(totals.reach), icon: TrendingUp },
              { label: "Clics", value: format(totals.clicks), icon: MousePointerClick },
              { label: "CTR", value: avgCtr.toFixed(1) + "%", icon: BarChart3 },
              { label: "Ventes", value: String(totals.sales), icon: ShoppingCart },
              { label: "ROAS", value: totalRoas.toFixed(1) + "x", icon: DollarSign },
            ].map(stat => (
          ...
          {(() => {
            const chartData = insights.length > 0 ? insights : MOCK_INSIGHTS;
            return (
          <>
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
            <p className="text-xs text-amber-700">
              💡 Les données d&apos;insights sont actuellement en mode <strong>{insights.length > 0 ? "réelles (Meta API)" : "démonstration"}</strong>.
              {insights.length === 0 && " Connectez un compte Meta Business actif pour voir vos vraies performances."}
            </p>
          </div>
          </>);
          })()}
