    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-gray-500 mt-0.5">Vue d&apos;ensemble de votre boutique</p>
        </div>
        <div className="flex items-center gap-1.5 sm:gap-2">
          <div className="flex items-center bg-white rounded-lg border border-gray-200 p-0.5 overflow-x-auto">
            {dateFilterOptions.map((opt) => (
              <button key={opt.value} onClick={() => setDateFilter(opt.value as any)}
                className={`px-2 sm:px-3 py-1 sm:py-1.5 rounded-md text-[10px] sm:text-xs font-medium whitespace-nowrap transition-colors ${dateFilter === opt.value ? "bg-gray-900 text-white" : "text-gray-500 hover:text-gray-900"}`}>
                {opt.label}
              </button>
            ))}
          </div>
          <button onClick={fetchData} className="p-1.5 sm:p-2 bg-white rounded-lg border border-gray-200 hover:bg-gray-50 transition-colors">
      ...
      <div className="grid grid-cols-2 gap-2 sm:gap-4 lg:gap-6">
        {stats.map((s, i) => (
          <div key={i} className="bg-white rounded-xl border border-gray-200 p-3 sm:p-5 hover:shadow-sm transition-shadow animate-fade-in" style={{ animationDelay: `${i*0.05}s` }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-6">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-3 sm:p-6">
          <h3 className="text-xs sm:text-sm font-semibold text-gray-900 mb-3 sm:mb-4">Tendance des ventes</h3>
        ...
        {/* Status Distribution */}
        <div className="bg-white rounded-xl border border-gray-200 p-3 sm:p-6">
          <h3 className="text-xs sm:text-sm font-semibold text-gray-900 mb-3 sm:mb-4">Répartition des commandes</h3>
          ...
          <div className="flex flex-wrap justify-center gap-2 sm:gap-3 mt-3 sm:mt-4">
            {statusDistribution.map((s) => (
              <div key={s.name} className="flex items-center gap-1.5">
                <div className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full" style={{ backgroundColor: s.color }}></div>
                <span className="text-[10px] sm:text-xs text-gray-600">{s.label} ({s.value})</span>
              </div>
            ))}
          </div>
        </div>
