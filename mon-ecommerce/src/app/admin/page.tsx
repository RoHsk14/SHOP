...
import {
  ShoppingCart, Euro, TrendingUp, TrendingDown, RefreshCw, ArrowUpRight, ArrowDownRight,
...
export default function AdminDashboard() {
  ...
  const fetchData = async () => {
    ...
    const [ordersRes, prevOrdersRes, productsRes, visitsRes, prevVisitsRes] = await Promise.all([
      ...
      supabase.from("orders").select("*").gte("created_at", prevStart.toISOString()).lte("created_at", prevEnd.toISOString()),
      supabase.from("products").select("*"),
      fetch(`/api/visitors?type=visits&start=${encodeURIComponent(start)}&end=${encodeURIComponent(end)}`),
      fetch(`/api/visitors?type=visits&start=${encodeURIComponent(prevStart.toISOString())}&end=${encodeURIComponent(prevEnd.toISOString())}`)
    ]);
  ...
  const mostDiscounted = products
    ...
    .sort((a, b) => ((a.regular_price || 0) - (a.sale_price || 0)) - ((b.regular_price || 0) - (b.sale_price || 0)))
    .slice(0, 5);
  ...
  return (
    <div className="space-y-4 sm:space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-6">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-3 sm:p-6 hover:shadow-md transition-shadow">
          <div className="flex items-center justify-between mb-3 sm:mb-4">
            <h3 className="text-xs sm:text-sm font-semibold text-gray-900">Produits les plus discountés</h3>
            <span className="text-[10px] sm:text-xs text-gray-400 bg-gray-50 px-2 py-1 rounded-md">
          ...
          <div className="space-y-2 sm:space-y-3">
            {mostDiscounted.length === 0 ? (
              <p className="text-gray-400 text-xs text-center py-6">Aucun produit discounté</p>
            ) : (
              mostDiscounted.map((product, i) => {
                ...
                return (
                  <div key={product.id}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        <span className="text-[10px] text-gray-400">{product.sku || `ID: ${product.id.slice(0, 8)}`}</span>
                    ...
                    <div className="text-right shrink-0">
                      <p className="text-xs sm:text-sm font-bold text-gray-900">
                        {product.sale_price ? formatPrice(product.sale_price, defaultCurrency) : "—"}
                      </p>
