...
export async function POST(request: NextRequest) {
  try {
    const { access_token, business_id, catalog_id } = await request.json();

    if (!access_token || !business_id) {
      return NextResponse.json({ error: "access_token and business_id required" }, { status: 400 });
    }

    let activeCatalogId = catalog_id;
