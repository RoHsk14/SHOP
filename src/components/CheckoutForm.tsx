  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="bg-gray-50 rounded-xl p-4 sm:p-5 mb-5 border border-gray-100">
        <div className="flex flex-col sm:flex-row sm:items-baseline justify-between gap-2 sm:gap-0 mb-4">
        ...
        <div className="pt-3 border-t border-gray-200 flex items-baseline justify-between">
          ...
          <span className="text-xl sm:text-2xl font-extrabold text-green-700">
            {formatPrice(totalPrice, productCurrency)}
          </span>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        {formFields.map((field) => (
          <div key={field.name}>
            ...
            <input
              ...
              className="w-full bg-white border border-gray-200 rounded-lg py-2.5 px-3 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-colors"
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full bg-green-600 hover:bg-green-700 active:bg-green-800 text-white font-semibold py-3 sm:py-3.5 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed mt-2 text-sm sm:text-[15px]"
        >
          {loading ? (
